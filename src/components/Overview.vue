<template>
  <v-container>
    ...
    <h1>Uren overzicht</h1>
		<div class="text-xs-center"><h2>{{ date | displayDate }}</h2></div>
    <v-data-table
      :headers="headers"
      :items="uren"
      hide-actions
		...
		<v-layout row>
			<v-flex xs6>
				<div class="text-xs-left">
					<v-btn v-if="prevBtn" color="primary" @click="prevMonth">Vorige</v-btn>
				</div>
			</v-flex>
			<v-flex xs6>
				<div class="text-xs-right">
					<v-btn v-if="nextBtn" color="primary" @click="nextMonth">Volgende</v-btn>
				</div>
			</v-flex>
		</v-layout>
    ...
    >
      ...
      <v-btn flat color="pink" @click.native="deleted = false">Close</v-btn>
		...
		<div class="text-xs-left"><h3>Totaal: {{ monthTotal }}</h3></div>
  </v-container>
</template>
